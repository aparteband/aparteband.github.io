...
function createLinesCanvas(stage) {
 ...
 function drawMaskOutsideVideo() {
  ...
  // Это нужно только чтобы сохранить прежний размер:
  // ширина “вписывания” берётся такой же, как раньше у правой области.
  const rect = v.getBoundingClientRect();
  if (!rect || rect.width < 2 || rect.height < 2) return;

  const pad = (LINES_CFG.avoidVideoPadPx | 0) || 0;

  // Раньше “разрешённая область” была справа: от rect.right+pad до конца экрана.
  const rightRegionX = Math.min(w, Math.max(0, Math.ceil(rect.right) + pad));
  const rightRegionW = Math.max(0, w - rightRegionX - ((LINES_CFG.rightPadPx | 0) || 0));
  const allowedH = h;

  if (rightRegionW < 10 || allowedH < 10) return;

  // ✅ Теперь переносим ЭТУ ЖЕ ширину области влево (размер проекции не меняется)
  const allowedX = 0;
  const allowedW = rightRegionW;

  // источник маски
  const sw = mask.width, sh = mask.height;
  if (sw < 2 || sh < 2) return;

  // берём весь mask (как и раньше, если у тебя был crop — оставь его выше, тут не меняем)
  const sxS = 0, syS = 0, sWS = sw, sHS = sh;

  // масштаб — тот же принцип, что был (вписать в allowedW/allowedH + drawScale)
  ...
  // ✅ оставляем прежнюю “логику позиционирования внутри области”: центрирование,
  // просто область теперь слева (так ты не получишь неожиданный кроп/скачок композиции)
  ...
  // ✅ фейд теперь по ПРАВОМУ краю проекции/области (а не по левому)
