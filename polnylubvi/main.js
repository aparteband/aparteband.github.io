...
/* ============================================================
   ...
   Реализация:
   - Берём “линии” из кадра видео (luma/chroma key)
   - РИСУЕМ ИХ ТОЛЬКО ВНЕ ПРЯМОУГОЛЬНИКА ВИДЕО (справа от видео),
     с отступом avoidVideoPadPx.
   - Видео НЕ рисуется в canvas.

   ДОБАВЛЕНО (ускорение загрузки):
   - Первый доступный кадр фиксируем как статичный (без анимации),
     пока видео не будет достаточно загружено для плавного воспроизведения.
   - После этого включаем обновление по updateFps/holdFrames.

   Где регулировать:
   - drawScale: масштаб проекции (сейчас x2)
   - avoidVideoPadPx: отступ от видео (чтобы точно не касались)
   - fadeEdgePx: сглаживание левого края проекции
   - yOffsetPct: опустить проекцию вниз
   - bufferingPctToAnimate: какой % буфера считать “достаточно загружено”
============================================================ */
const LINES_CFG = Object.freeze({
  ...
  rightPadPx: 0,             // отступ справа, если нужен

  fadeEdgePx: 170,           // ширина “сглаживания” границы СЛЕВА (в px)
  yOffsetPct: 0.20,          // смещение вниз на 20% высоты области

  // “пока всё не загрузится”: считаем, что можно анимировать,
  // когда буфер >= bufferingPctToAnimate (если duration известен), либо canplaythrough
  bufferingPctToAnimate: 0.85
});
...
function createLinesCanvas(stage) {
  ...
  // оптимизация: не аллоцировать ImageData на каждый шаг
  let outImg = null;

  // статичный первый кадр
  let firstFrameCaptured = false;
  let canAnimate = false;         // станет true, когда “всё достаточно загрузилось”
  let gotCanPlayThrough = false;

  // чтобы не “тереть” холст, когда видео ещё не готово
  ...
  function ensureSampleSize(v) {
    ...
    if (src.width !== sw || src.height !== sh) {
      src.width = sw;
      src.height = sh;
      outImg = null; // размер изменился — пересоздадим буфер
    }
    ...
    if (!outImg || outImg.width !== sw || outImg.height !== sh) {
  ...
  function isBufferedEnough(v) {
    // canplaythrough — приоритет
    if (gotCanPlayThrough) return true;

    // readyState 4 — обычно достаточно
    if (v && v.readyState >= 4) return true;

    // если duration известна — оцениваем buffered
    try {
      const dur = v && Number.isFinite(v.duration) ? v.duration : NaN;
      ...
      const pct = end / dur;
      return pct >= (LINES_CFG.bufferingPctToAnimate || 0.85);
    } catch (_) {
      return false;
    }
  }
  ...
  function drawMaskOutsideVideo() {
    ...
    const allowedW = Math.max(0, w - allowedX - ((LINES_CFG.rightPadPx | 0) || 0));
    const allowedH = h;
    ...
    // размещаем “линии” на всей правой области, масштаб, лишнее обрезаем клипом
  ...
  function renderStaticIfPossible() {
    const v = getVideoEl();
    if (!v) return;

    // если первый кадр ещё не схвачен — пробуем сделать это при первой возможности
    if (!firstFrameCaptured) {
      const ok = buildMaskFromVideoFrame(v);
      if (ok) firstFrameCaptured = true;
    }

    // рисуем то, что есть (если ещё нет кадра — просто ничего не трогаем)
    if (firstFrameCaptured) {
      // не чистим холст: просто рисуем поверх (визуально стабильно)
  ...
  function renderLiveStep() {
    const v = getVideoEl();
    const ok = buildMaskFromVideoFrame(v);
    if (!ok) return; // не чистим — чтобы не мигало
    drawMaskOutsideVideo();
  }
  ...
  function loop() {
    ...
    // оптимизация: если вкладка скрыта — не гоняем рендер
    if (document.hidden) {
    ...
    const v = getVideoEl();
    if (v && !canAnimate) {
      // включаем анимацию только когда “достаточно загрузилось”
      if (isBufferedEnough(v)) canAnimate = true;
    }

    if (!canAnimate) {
      // статичный первый кадр
      renderStaticIfPossible();
      rafId = requestAnimationFrame(loop);
      return;
    }

    // live: покадровое обновление
    ...
    if (step !== lastStep) {
      ...
      // ничего не делаем — экономим CPU
      if (!hasDrawnSomething) {
        // если вдруг ещё не рисовали (очень редко), попытаемся статично
        renderStaticIfPossible();
      }
    }
...
(function build() {
  ...
  // видео должно быть “невидимым”, но оставаться источником кадров
  ...
  // фиксируем поведение плашки (без зависимости от --vh)
  ...
  // затем делаем в 2 раза уже (как задано)
  ...
  function updatePinkPanel() {
    try {
      ...
      // проекция: справа от rect.right + pad
