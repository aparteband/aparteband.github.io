...
const LINES_CFG = Object.freeze({
  ...
  count: 4,               // как на твоём референсе
  lineWidthPx: 4,
  baseAmpPx: 58,
  wavelengthPx: 320,
  speed: 0.55,            // 0.3 медленно / 0.6 средне / 1.0 быстро
  cycleSec: 14,           // период “выпрямления”
  jitterPx: 1.1,          // дрожь штриха
  topBottomOverscanPx: 80 // чтобы не было дыр на краях
});
...
function createLinesCanvas(stage) {
  ...
  // фиксированные фракции X, близко к твоей картинке
  const xFracs = [0.16, 0.385, 0.505, 0.76];

  const lines = xFracs.slice(0, LINES_CFG.count).map((xf, i) => ({
    xf,
    p1: (i * 1.9) + 0.3,
    p2: (i * 2.6) + 1.1,
    s:  0.7 + i * 0.08
  ...
  function draw(tSec) {
    ...
    const step = 26; // шаг по Y (производительность/плавность)

    for (let i = 0; i < lines.length; i++) {
      const L = lines[i];
      const xBase = L.xf * w;

      // амплитуда уходит к 0 => линия становится прямой
      ...
      for (let y = y0; y <= y1; y += step) {
        ...
        // двухчастотная волна, как органический штрих
        const wave =
          Math.sin(k1 * y + tt + L.p1) +
          0.35 * Math.sin(k2 * y - tt * 0.9 + L.p2);

        const x = xBase + (amp * 0.72 * wave) + jitter;
