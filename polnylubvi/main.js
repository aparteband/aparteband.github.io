...
/* ============================================================
   ...
   ТЗ: “один в один” ТОЛЬКО линии с видео.
   - Видео НЕ рисуется в canvas.
   - Из текущего кадра <video> берём только “линии” (luma/chroma key).
   - Накладываем ТОЛЬКО на область видео на экране, с учётом:
     object-fit: cover + object-position.

   ВАЖНОЕ ИСПРАВЛЕНИЕ:
   - Убираю desktopMinWidth-ограничение (оно часто выключает эффект на ПК при узком окне).
   - Включаем эффект везде, КРОМЕ mobile-coarse (как у твоего CSS @media hover:none/pointer:coarse).

   РЕГУЛИРОВКА:
   - updateFps / holdFrames: покадровость
   - lumaThreshold: порог белого (выше = меньше линий)
   - softness: мягкость порога
   - chromaMax: насколько “строго белое”
============================================================ */
const LINES_CFG = Object.freeze({
  updateFps: 24,
  holdFrames: 3,             // 24/3 = 8 обновлений/сек (покадрово)

  sampleMaxW: 520,           // чуть больше деталей (если тяжело — верни 420)
  lumaThreshold: 155,        // ✅ было 205 — стало ниже, чтобы линии точно появились
  softness: 120,             // ✅ мягче порог, чтобы брать антиалиас
  chromaMax: 90,             // ✅ менее строго, чтобы не “съедало” линии

  alpha: 1.0                 // ✅ на время отладки ставим 1.0
});

function shouldRunLines() {
  // НЕ трогаем мобилу: отключаем на (hover:none & pointer:coarse)
  try {
    if (!window.matchMedia) return true;
    const isMobileCoarse = window.matchMedia("(hover: none) and (pointer: coarse)").matches;
    return !isMobileCoarse;
  } catch (_) {
    return true;
  }
}
...
function createLinesCanvas(stage) {
  ...
  // страховка, даже если CSS не применился/перебит
  canvas.style.position = "fixed";
  canvas.style.inset = "0";
  canvas.style.pointerEvents = "none";
  canvas.style.zIndex = "2";          // ✅ чтобы было поверх видео (если у видео z-index=1)
  canvas.style.opacity = "1";
  canvas.style.mixBlendMode = "screen"; // ✅ белые линии выглядят как “световые” поверх
  ...
  function ensureSampleSize(v) {
    ...
    const sw = Math.max(160, Math.min(LINES_CFG.sampleMaxW | 0, vw));
    const sh = Math.max(160, Math.round(sw * (vh / vw)));
  ...
  function drawMaskOnVideo() {
    ...
    // размеры layout-бокса (без transform) — для cover-crop
