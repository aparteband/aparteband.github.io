...
settleToTopSec: 6          // ✅ за сколько секунд “видео” опустится так, что верх = 0
});
...
function createLinesCanvas(stage) {
  ...
  let settleStartMs = 0; // старт плавного опускания (после freeze)
  ...
  function drawMaskOutsideVideo() {
    ...
    // смещение вниз    const yShift = allowedH * (LINES_CFG.yOffsetPct || 0);
    const dyStart = (allowedH - dh) * 0.5 + yShift; // текущее (как было)

    // плавно “опускаем” dy к 0, чтобы верх проекции совпал с верхом экрана
    const durMs = Math.max(1, (LINES_CFG.settleToTopSec || 1) * 1000);
    const tMs = settleStartMs ? (performance.now() - settleStartMs) : 0;
    const p = Math.max(0, Math.min(1, tMs / durMs));
    const dy = dyStart + (0 - dyStart) * p;
  ...
  freezeMode = false;
  if (!settleStartMs) settleStartMs = performance.now();
}, { passive: true });
