...
/* ============================================================
   ...
   Покадровое ощущение (frame-by-frame): квантуем время по FPS,
   “перерисовываем” контур каждый кадр (boil), без плавной интерполяции.

   РЕГУЛИРОВКА ПАРАМЕТРОВ ТУТ:
   - frameFps:      “покадровый” FPS (8 = более рвано, 12 = норм, 15 = мягче)
   - speed:         скорость движения (больше = быстрее)
   - boilPx:        сила “перерисовки” контура на кадр
   - yStepPx:       шаг по Y (меньше = больше точек = глаже, но тяжелее)
...
function createLinesCanvas(stage) {
  ...
  // дешёвый детерминированный шум 0..1 (для “boil” покадрово)
  function hash01(n) {
    n = (n | 0) + 0x6D2B79F5;
  ...
  // рисуем ОДИН “кадр” по номеру frame (покадрово)
  function drawFrame(frame) {
    ctx.clearRect(0, 0, w, h);

    const fps = Math.max(1, LINES_CFG.frameFps);
    const tSec = frame / fps; // квантуем время
    ...
    const step = Math.max(2, LINES_CFG.yStepPx);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // “кадровый” сдвиг всей линии (как неряшливость покадровки)
      const frameShift = (hash01(frame * 97531 + i * 101) - 0.5) * 2 * LINES_CFG.frameShiftPx;
      ...
      for (let y = y0; y <= y1; y += step) {
        ...
        // “перерисовка” на кадр: зависит от frame + линии + индекса по Y
        const yIdx = ((y - y0) / step) | 0;
        const boil = (hash01(frame * 1315423911 + i * 374761393 + yIdx * 668265263) - 0.5) * 2 * LINES_CFG.boilPx;

        let x = xBase + (amp * 0.72 * wave) + jitter + boil + frameShift;
      ...
      // сглаживание (quadratic через midpoints) — геометрия гладкая, но смена кадров покадровая
      ctx.beginPath();
  ...
  function loop() {
    ...
    const frame = Math.floor(tSec * Math.max(1, LINES_CFG.frameFps));

    // перерисовка только при смене “кадра” => покадровое ощущение
    if (frame !== lastFrame) {
      lastFrame = frame;
      drawFrame(frame);
    }
