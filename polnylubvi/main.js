...
(function build() {
  ...
  panel.style.zIndex = "10050";          // ✅ выше canvas (у тебя canvas 9999)
  panel.style.transform = "none";
  panel.style.borderRadius = "0";        // ✅ прямые углы
  // фон оставляем как был (если CSS уже задаёт — можно не трогать; но так надёжнее)
  ...
  const OVERLAP_PCT = 0.10; // <-- тут регулируешь заход на проекцию

  function getLayoutWidth() {
    // ширина должна совпадать с тем, относительно чего считаются rect'ы
  ...
  function updatePinkPanel() {
    try {
      ...
      // ширина плашки = вся левая часть + 10% проекции
