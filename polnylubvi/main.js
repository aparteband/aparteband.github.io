...
(function build() {
  ...
  // ✅ возвращаем розовую плашку (без ссылок/тайтла)
  const panel = document.createElement("div");
  panel.id = "pink-panel";
  panel.setAttribute("aria-hidden", "true");

  // ✅ позиционирование плашки через inline, чтобы не править CSS:
  // слева, на всю высоту экрана
  panel.style.position = "fixed";
  panel.style.left = "0";
  panel.style.top = "0";
  panel.style.height = "calc(100 * var(--vh, 1vh))";
  ...
  // (проекция у тебя сейчас определяется как область справа от rect.right + avoidVideoPadPx)
  const OVERLAP_PCT = 0.10; // <-- тут регулируешь “заход” на проекцию (10%)
  ...
  function updatePinkPanel() {
    try {
      ...
      const allowedW = Math.max(0, ww - allowedX - ((LINES_CFG.rightPadPx | 0) || 0));

      // ширина плашки = “левая часть” до проекции + 10% проекции
      const wPx = Math.max(0, Math.min(ww, Math.round(allowedX + allowedW * OVERLAP_PCT)));
