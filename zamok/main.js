/* ============================================================
   ...
   — FIX: address bar не дергает сцену после старта
   — FIX: ориентация адаптивна (портрет/ландшафт имеют свои baseline vh)
============================================================ */
(function () {
  if (window.__vhFixV4) return;
  window.__vhFixV4 = true;
  ...
  function ensureFrozenFor(ori) {
    if (!window.__appStarted) return false;
    if (!frozenByOri[ori]) {
  ...
  function endSettle(commitOri) {
    ...
    // чтобы иконки сразу пересчитались под новый vh (без ожидания следующего resize)
    try {
      if (typeof window.__repositionIconsFan === "function") window.__repositionIconsFan();
    } catch (_) {}
  }
  ...
  function settleLoop() {
    ...
    // окно стабилизации: достаточно длинное для in-app (Telegram) после поворота
    if (dt >= 750) {
      endSettle(settleOri);
      settleRAF = 0;
      return;
  ...
  function onResizeLike() {
    ...
    if (settling) {
      // settleLoop сам применяет максимум, но на всякий случай обновим max
      if (h > settleMax) settleMax = h;
      return;
    }

    // До старта: живём "по максимуму" (не уменьшаем)
  ...
  window.addEventListener("pageshow", () => {
    const ori = getOriKey();
    if (window.__appStarted) {
      // на in-app pageshow может быть частью “перехода” после поворота/поиска
      beginSettle(ori);
    } else {
      onResizeLike();
    }
  ...
  window.addEventListener("orientationchange", () => {
    const after = () => {
      ...
      if (!window.__appStarted) {
        // до старта — просто пересчитать максимум
        onResizeLike();
        return;
      }

      // после старта — адаптивно к ориентации, но через "окно максимума"
      beginSettle(ori);
    };

    // чуть позже — чтобы WebView успел выдать реальные размеры
    setTimeout(after, 60);
  });
})();
...
/* ВАЖНО: если ниже по коду где-то используется __STAR_MOBILE_SCALE — эта строка должна быть */
const __STAR_MOBILE_SCALE = __IS_MOBILE_UI ? 2 : 1;

function __getStableViewportH() {
  ...
  return Math.max(1, oneVhPx * 100);
}

function __getStableViewportW() {
  if (window.visualViewport && typeof window.visualViewport.width === "number") {
    return Math.max(1, window.visualViewport.width);
  }
  return Math.max(1, document.documentElement.clientWidth || window.innerWidth);
}

/* ============================================================
   MOBILE ICONS LAYOUT (ONLY MOBILE)
   - ниже на 18px (см. showIconsFan)
   - gap между иконками меньше на 30% (см. showIconsFan)
   - иконки меньше на 10% (см. showIconsFan)
   - центрирование симметрией относительно cx
   - учитываем “видимую” высоту (iOS панель/поисковая строка)
============================================================ */
/* ============================================================
   MOBILE ICONS LAYOUT (ONLY MOBILE) — FOR STARS SPAWN (NO CONFLICT)
   (нужна createStars() для no-star zone снизу)
============================================================ */
function __stars_computeMobileIconsLayout() {
  if (!__IS_MOBILE_UI) return null;

  // ВАЖНО: no-star zone должен использовать ТОЧНО ту же геометрию,
  // что и реальные иконки (showIconsFan), иначе на разных экранах возможны касания.
  const lay = __iconsFan_computeMobileLayout();
  if (!lay || !isFinite(lay.size) || !isFinite(lay.top)) return null;
...
/* ============================================================
   ...
   ВАЖНО: это безопасная вставка: async, try/catch, сайт не ломает
============================================================ */
(function () {
  if (window.__goatcounterInjectV1) return;
  window.__goatcounterInjectV1 = true;

  // 1) Вставь сюда свой код из GoatCounter:
  // В интерфейсе GoatCounter он выглядит как:
  // data-goatcounter="https://MYCODE.goatcounter.com/count"
  var GC_ENDPOINT = "https://MYCODE.goatcounter.com/count"; // <-- заменить MYCODE

  try {
    // 2) Подключаем count.js динамически (чтобы НЕ трогать HTML)
  ...
  // 3) Хелпер для событий (клики по иконкам)
  // GoatCounter: event:true + path НЕ должен начинаться с "/" :contentReference[oaicite:5]{index=5}
  window.__gcEvent = function (name, title) {
    try {
...
function createStars() {
  ...
  const w = __IS_MOBILE_UI ? __getStableViewportW() : window.innerWidth;
  const h = __IS_MOBILE_UI ? __getStableViewportH() : window.innerHeight;

  // базовая “нижняя зона”, которая уже была
  const baseNoStar = Math.max(90, h * 0.1);

  // по умолчанию — как раньше
  let maxStarY = h - baseNoStar;

  // ONLY MOBILE: если иконки перекрывают звёзды — усиливаем “no-star zone” снизу
  // ONLY MOBILE: если иконки перекрывают звёзды — усиливаем “no-star zone” снизу
  if (__IS_MOBILE_UI) {
    const lay = __stars_computeMobileIconsLayout();

    // защита от NaN/undefined (это и было источником “звёзды у верхнего края”)
    if (lay && isFinite(lay.size) && isFinite(lay.top)) {
      // небольшая “прокладка” над иконками, чтобы звёзды не лезли под них
      const guard = Math.round(Math.max(10, lay.size * 0.25));

      // сколько пикселей снизу надо зарезервировать, чтобы верхняя граница зоны была (lay.top - guard)
      const iconsNoStar = Math.max(0, h - (lay.top - guard));

      // увеличиваем зону ТОЛЬКО если нужно (т.е. если иконки реально выше базовой зоны)
      const noStar = Math.max(baseNoStar, iconsNoStar);

      window.__iconsNoStarZonePx = noStar; // отладка
      maxStarY = Math.min(maxStarY, h - noStar);
    }
  }


  maxStarY = Math.max(0, maxStarY);
...
/* стабильные размеры viewport (для iOS с панелями) */
function __iconsFan_getViewportW() {
  if (window.visualViewport && typeof window.visualViewport.width === "number") {
    return window.visualViewport.width;
  }
  return document.documentElement.clientWidth || window.innerWidth;
}
function __iconsFan_getViewportH() {
  // приоритет — твой vh-fix (visualViewport.height -> --vh)
  const vh = parseFloat(
    getComputedStyle(document.documentElement).getPropertyValue("--vh")
  );
  if (isFinite(vh) && vh > 0) return vh * 100;
...
/* мобильная раскладка:
   - размер иконки: 0.8*(W/6) = W/7.5 (это как раз “на ~20% меньше” от максимально возможного при полях)
   - gap считаем из условия: W = 6S + 4G  (поля ~ 0.5S слева/справа)
*/
/* ============================================================
   MOBILE ICON FAN TUNING (EDIT HERE)
============================================================ */
const ICON_FAN_MOBILE_SIZE_K = 1.2; // размер иконок на телефоне
const ICON_FAN_MOBILE_GAP_K  = 1.05; // расстояние между иконками на телефоне

function __iconsFan_computeMobileLayout() {
  const W = Math.max(1, __iconsFan_getViewportW());
  const H = Math.max(1, __iconsFan_getViewportH());
  const n = ICONS.length;

   let S = Math.round(Math.max(56, Math.min(96, W / 6.8)));
  S = Math.round(S * ICON_FAN_MOBILE_SIZE_K);

  let G = Math.round(Math.max(10, Math.min(22, S * 0.55)));
  G = Math.round(G * ICON_FAN_MOBILE_GAP_K);


  const step = S + G;

   // базовый отступ снизу (видимая высота: --vh/visualViewport)
  let bottomPad = Math.round(Math.max(24, S * 1.05));
  let top = Math.round(H - (S + bottomPad));

  // ТЗ: на мобиле опустить иконки ниже на 10px
  top = top + 22 + 10;

  // страховка
  const minTop = Math.round(H * 0.55);
  const maxTop = Math.round(H - S - Math.max(8, Math.min(36, S * 0.35)));
  ...
  // ВАЖНО: position fixed + left задаётся как ЛЕВЫЙ КРАЙ,
  // поэтому для центрирования фан-раскладки нужен baseLeft = cx - S/2
  const baseLeft = Math.round(cx - S / 2);

  return { W, H, n, size: S, gap: G, step, cx, baseLeft, top };
}


function __iconsFan_applyToIcon(icon, idx, layout, mobile) {
  if (!icon || !icon.isConnected) return;
  if (icon.classList.contains("icon-exit")) return;

  // убираем конфликт top+bottom (в CSS есть bottom: 40px)
  icon.style.bottom = "";
  ...
  if (mobile) {
    if (!layout || !isFinite(layout.size) || !isFinite(layout.top) || !isFinite(layout.step)) return;

    icon.style.width = Math.round(layout.size) + "px";
    icon.style.left = Math.round(layout.baseLeft) + "px";
    icon.style.top = Math.round(layout.top) + "px";

    icon.style.setProperty("--tx", Math.round(offset * layout.step) + "px");
    icon.style.setProperty("--rot", (offset * 12) + "deg");
  }
}


/* публичный “перерасчёт” (нужен для visualViewport resize) */
window.__repositionIconsFan = function () {
  // только для мобилы
  if (!__IS_MOBILE_UI) return;

  const icons = Array.from(document.querySelectorAll(".icon"));
  if (!icons.length) return;

  const layout = __iconsFan_computeMobileLayout();

  for (let i = 0; i < icons.length; i++) {
    const ic = icons[i];
    if (!ic || !ic.isConnected) continue;

    // индекс берём из dataset (если есть), иначе по порядку
    const idxRaw = ic.dataset ? ic.dataset.fanIndex : null;
    const idx = (idxRaw != null && idxRaw !== "") ? (parseInt(idxRaw, 10) || 0) : i;

    // если скрыта — не трогаем
    const cs = getComputedStyle(ic);
    if (cs.display === "none") continue;

    __iconsFan_applyToIcon(ic, idx, layout, true);
  }
};

function showIconsFan() {
  const isMobile = __IS_MOBILE_UI;

  // DESKTOP (как было)
  let baseLeft = window.innerWidth / 2;   // на десктопе оставляем старое поведение
  ...
  let mobileLay = null;
  if (isMobile) {
    mobileLay = __iconsFan_computeMobileLayout();
    if (mobileLay) {
      baseLeft = mobileLay.baseLeft;      // ВАЖНО: левый край, а не центр
      y = mobileLay.top;
    }
  }
  ...
  ICONS.forEach((src, i) => {
    ...
    icon.src = src;
    icon.className = "icon";
    icon.dataset.fanIndex = String(i);
    ...
    if (isMobile) {
      ...
      if (mobileLay && isFinite(mobileLay.size)) {
        icon.style.width = mobileLay.size + "px";
      }

      if (mobileLay && isFinite(mobileLay.step)) {
        icon.style.setProperty("--tx", Math.round(offset * mobileLay.step) + "px");
      } else {
...
/* ---------- ICON FAN: AUTO REPOSITION ON MOBILE (visualViewport-safe) ---------- */
(function () {
  if (window.__iconsFanAutoRepositionV1) return;
  window.__iconsFanAutoRepositionV1 = true;
  ...
  function schedule() {
    ...
    raf = requestAnimationFrame(() => {
      raf = 0;
      try {
        if (typeof window.__repositionIconsFan === "function") {
          window.__repositionIconsFan();
        }
      } catch (_) {}
    });
  }
...
/* ---------- ПЕРЕХОД ПО ССЫЛКАМ ---------- */
const ICON_LINKS = {
  sp: "https://mts-music-spo.onelink.me/sKFX/qgf6096r",
  zv: "https://share.zvuk.com/cLQ0/n6daeqa1",
  ap: "https://music.apple.com/ru/album/%D0%B7%D0%B0%D0%BC%D0%BE%D0%BA-ep/1867579849",
  vk: "https://vk.ru/music/album/-2000361417_26361417_698baa2b108f724eeb",
  ya: "https://music.yandex.ru/album/40106109?utm_source=web&utm_medium=copy_link"
};
...
document.addEventListener(__TAP_EVT, e => {
  ...
  if (src.includes(key)) {
    ...
    // --- GoatCounter event: клик по конкретной иконке ---
    // path не должен начинаться с "/" :contentReference[oaicite:6]{index=6}
    try {
      if (typeof window.__gcEvent === "function") {
        window.__gcEvent("click-icon-" + key, "Icon click: " + key);
      }
    } catch (_) {}
...
(function initMiniHeroScene() {
  let heroMini = document.getElementById("hero-mini");
  if (!heroMini) {
    heroMini = document.createElement("img");
  ...
  if (!sled) {
    sled = document.createElement("img");
    sled.id = "sled";
    sled.src = "sled.png";
    document.body.appendChild(sled);
  }

  /* FIX: мини-герой при первом появлении на горе чуть выше санок (in-app тоже).
     Гарантированно опускаем только #hero-mini на +5px через inline top.
     Никаких изменений логики/таймингов/анимаций. */
  try {
    const isTouch =
    ...
    if (isTouch) {
      heroMini.style.top = "calc(5.5 * var(--vh, 1vh) + 20px)";
    }
  } catch (_) {}
})();
...
(function showHeroAfterMountain() {
  ...
  mountain.addEventListener("transitionend", (e) => {
    ...
    // FIX: игнорируем transform-transition, если реальный полёт сцены не активен
    // (в in-app изменение --vh могло дергать transform и давать ложный transitionend)
    if (!document.body.classList.contains("scene-fly-up")) return;
...
(function rideDownGrowSwapAndRebound() {
  ...
  const LANDING_SHIFT_Y = 0;
    ...
    // FIX: микро-посадка мини-героя на санки (только мобильный UI)
  // ТЗ: опустить мини-героя ещё на 5px ниже на мобиле
  const HERO_ON_SLED_DY_PX = __IS_MOBILE_UI
    ? (Math.max(5, Math.min(10, Math.round(Math.min(window.innerWidth, window.innerHeight) * 0.012))) + 5)
    : 0;
  ...
  function frame(ts) {
    ...
    if (state === "ride") {
      ...
      const s = 1 + (targetScale - 1) * q;
            ...
            Rider.hero.style.transform =
        base + " translate(0px, " + HERO_ON_SLED_DY_PX + "px) scale(" + s + ")";
