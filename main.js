/* ============================================================
   ...
   — FIX: "sticky vh" (не уменьшаем --vh при адресной строке/клавиатуре)
============================================================ */
(function () {
  if (window.__vhFixV2) return;
  window.__vhFixV2 = true;
  ...
  function setVhVarSticky(reset) {
    const h = readH();

    if (reset || !maxH) {
      maxH = h;
    } else {
      // ключевой момент: НЕ уменьшаем --vh при временных просадках высоты
      if (h > maxH) maxH = h;
    }
  ...
  // обычные resize/scroll у visualViewport могут приходить из-за адресной строки/клавиатуры
  // поэтому используем sticky-обновление (без уменьшения)
  window.addEventListener("resize", () => setVhVarSticky(false));
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", () => setVhVarSticky(false));
    window.visualViewport.addEventListener("scroll", () => setVhVarSticky(false));
  }

  // ориентация — это реальная смена геометрии, тут надо сбросить maxH
  window.addEventListener("orientationchange", () => {
    setTimeout(() => setVhVarSticky(true), 60);
  });

  // возврат на вкладку/страницу — тоже сброс (на iOS значения могут поменяться)
  window.addEventListener("pageshow", () => setVhVarSticky(true));
})();
...
/* ============================================================
   IN-APP ADDRESS BAR / FOCUS GUARD
   — ставим hardStartGate при уходе в адресную строку
   — снимаем только после стабилизации
============================================================ */
(function () {
  if (window.__inAppFocusGuardV1) return;
  window.__inAppFocusGuardV1 = true;

  let releaseTimer = 0;

  function armGate() {
    if (window.__hardStartGate && typeof window.__hardStartGate.arm === "function") {
      window.__hardStartGate.arm();
    }
  }

  function releaseGateDelayed() {
    if (releaseTimer) clearTimeout(releaseTimer);
    releaseTimer = setTimeout(() => {
      if (window.__hardStartGate && typeof window.__hardStartGate.release === "function") {
        window.__hardStartGate.release();
      }
    }, 300); // даём WebView “успокоиться”
  }

  // уход в адресную строку / поиск / системный UI
  document.addEventListener("visibilitychange", () => {
    if (document.hidden) {
      armGate();
    } else {
  ...
  // in-app браузеры любят дергать pageshow
  window.addEventListener("pageshow", () => {
    releaseGateDelayed();
  });

  // страховка: потеря фокуса
  window.addEventListener("blur", armGate);
  window.addEventListener("focus", releaseGateDelayed);
})();
