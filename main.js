...
function __stars_computeMobileIconsLayout() {
  ...
  const W = __getStableViewportW();
  const H = __getStableViewportH();
  const n = ICONS.length;

  // базовый размер от ширины (под телефон), дальше уменьшаем по ТЗ
  let S = Math.round(Math.max(56, Math.min(96, W / 7.5)));
  S = Math.round(S * 0.90); // ТЗ: -10%

  // базовый gap от размера, дальше уменьшаем по ТЗ
  let G = Math.round(Math.max(10, Math.min(22, S * 0.55)));
  G = Math.round(G * 0.70); // ТЗ: -30%

  const step = S + G;

  // базовый отступ снизу (в видимом окне)
  ...
  // страховка от выхода за “видимый” низ/верх
...
(function rideDownGrowSwapAndRebound() {
  ...
  const HERO_ON_SLED_DY_PX = __IS_MOBILE_UI
    ? Math.max(5, Math.min(10, Math.round(Math.min(window.innerWidth, window.innerHeight) * 0.012)))
    : 0;
