...
/* ============================================================
   ...
   — iOS/Safari/in-app: после сворачивания/возврата AudioContext может стать suspended
   — пытаемся resume на событиях “возврата” и на первом пользовательском жесте
============================================================ */
(function () {
  ...
  // возвраты/фокус — может не сработать без жеста, но не мешает
  ...
  // жест пользователя — максимально вероятный “разлок” resume на iOS
  const GESTURE_EVT =
...
(function () {
  ...
  let appliedForThisLift = false;
    ...
    // Важно: используем одну систему координат
    if (vv && typeof vv.width === "number" && typeof vv.height === "number") {
      return {
        W: Math.max(1, vv.width),
        H: Math.max(1, vv.height),
        offX: (typeof vv.offsetLeft === "number") ? vv.offsetLeft : 0,
        offY: (typeof vv.offsetTop === "number") ? vv.offsetTop : 0
      };
    }
  ...
  function applyMountainToSledContact() {
    ...
    // boundingClientRect в in-app может быть относительно visual viewport,
    // поэтому добавляем offsets (если они есть)
    const xS = (r.left + r.width * CONTACT_FX) + (vp.offX || 0);
    const yS = (r.top  + r.height * CONTACT_FY) + (vp.offY || 0);
    ...
    // STEP 7-min: если по ширине не хватает — масштабируем до ширины.
    // Это сохраняет возможность держать пик под санками и не уезжать влево/вправо.
    const minScaleByWidth = VW / IMG_W;
    if (isFinite(minScaleByWidth) && minScaleByWidth > 0 && (IMG_W * s) < VW) {
      s = minScaleByWidth;
    }
    ...
    // clamp X (теперь scaledW гарантированно >= VW)
