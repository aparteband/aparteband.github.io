...
(function () {
  ...
  /* ============================================================
     ...
     — порядок вызовов: клики/первый bell/ser+zam → остальные bell → прочее → gora
...
/* ============================================================
   STARS WRAP: безопасное получение/создание контейнера
   (чтобы старт/тапы не ломались, если #stars отсутствует в DOM на момент загрузки скрипта)
============================================================ */
let starsWrap = null;

function __ensureStarsWrapEl() {
  let el = document.getElementById("stars");
  if (el && el.isConnected) return el;

  // страховка: если в разметке нет #stars — создаём
...
function getStarsWrap() {
  if (starsWrap && starsWrap.isConnected) return starsWrap;
  starsWrap = __ensureStarsWrapEl();
  return starsWrap;
}
...
/* ============================================================
   STAR TAP STABILITY (MOBILE):
   - единый делегированный обработчик на document (pointerdown / touchstart)
   - минимизируем зависимость от пересоздания DOM и от особенностей iOS/in-app
============================================================ */
(function () {
  if (window.__starTapStabilityV1) return;
  window.__starTapStabilityV1 = true;

  // CSS-inject: убираем задержки/хайлайт/драг на тач-устройствах
  const STYLE_ID = "star-tap-stability-style";
  if (!document.getElementById(STYLE_ID)) {
    const st = document.createElement("style");
    st.id = STYLE_ID;
    st.textContent = `
      #stars{ touch-action: manipulation; -webkit-tap-highlight-color: transparent; }
      .star{
  ...
  // экспорт: можно дергать вручную при необходимости
  window.__STAR_TAP_EVT = EVT;
})();

/* ============================================================
   SAFE EXTERNAL OPEN (NO DOUBLE-NAV)
   — открываем через <a target="_blank"> без window.location fallback,
     чтобы не было “и в новой вкладке, и в текущей”
============================================================ */
...
/* ============================================================
   AUDIO: HEART (ser.mp3)
============================================================ */
const SER_SND = new Audio("ser.mp3");
...
/* ---------- SFX: ZAMOK (zam.mp3) ---------- */
const ZAM_SND = new Audio("zam.mp3");
...
function createStars() {
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    star.dataset.index = String(i); // важно для делегированного тапа
    star.className = "star";
...
(function () {
  ...
  const CL_BASE = new Audio(CL_SRC);
...
/* ============================================================
   STAR TAP HANDLER (делегированный, стабильный)
   - объединяет SFX + glow + special (castle/heart) + прогресс
============================================================ */
let totalStarsInitial = STAR_COUNT;
let starsActivated = 0;
let allStarsTriggered = false;

function __markStarActivatedOnce(star) {
  if (!star || !star.isConnected) return;

  if (WorldState && WorldState.runCompleted) return; // после финала прогресс не считаем
...
function __handleStarTap(star) {
  ...
  setTimeout(() => {
    if (star && star.isConnected) star.classList.remove("glow");
  }, 1200);

  // прогресс считаем для всех кликов по звёздам (до финала)
  __markStarActivatedOnce(star);

  // special FX (до финала)
  if (!progressLocked) {
...
// единый слушатель (pointerdown/touchstart) — максимально “живучий” на iOS/in-app
(function attachStableStarTap() {
  if (window.__stableStarTapAttachedV1) return;
  window.__stableStarTapAttachedV1 = true;
  ...
  document.addEventListener(EVT, (e) => {
    ...
    // мышь: только левая кнопка
    if (e && e.type === "pointerdown" && e.pointerType === "mouse") {
      if (typeof e.button === "number" && e.button !== 0) return;
    }

    if (e && e.cancelable && typeof e.preventDefault === "function") e.preventDefault();

    __handleStarTap(star);
...
(function () {
  ...
  /* ============================================================
     FIX START 0.2:
     - если #boot-overlay уже есть в DOM (от старой версии/разметки),
       гарантируем что внутри есть #boot-start-btn
  ============================================================ */
  function ensureOverlay() {
    ...
    let btn = ov.querySelector("#" + BTN_ID);
    if (!btn) {
  ...
  function startFirstScene() {
    ...
    /* IMPORTANT FIX: на мобильных НЕ пересоздаём звёзды на resize (адресная строка/visualViewport ломают тапы) */
  ...
  function onStart(e) {
    ...
    /* ============================================================
       FIX START 0.1:
       снимаем hard-start gate прямо здесь, ДО removeOverlay(),
       чтобы сцена не оставалась скрытой/замороженной.
    ============================================================ */
    try {
    ...
    try { document.documentElement.removeAttribute("data-prestart"); } catch (_) {}
  ...
  if (btn) {
    ...
    /* ============================================================
       FIX START 0.3: страховка — вешаем и click, и pointerup
       (started защищает от двойного запуска)
    ============================================================ */
    if (__TAP_EVT !== "click") {
      btn.addEventListener("click", onStart, { passive: false });
    }
    if (__TAP_EVT !== "pointerup") {
      btn.addEventListener("pointerup", onStart, { passive: false });
    }
...
/* ============================================================
   ...
   - NO resizing of hero/sled.
   - gora.png peak (287,158) -> sled contact point (140,320) in sled.png (512x512).
   - Scale rule: from peak down to screen bottom (crop allowed), no "contain".
   - Trigger: when body gets class "scene-fly-up" (start of mountain scene).
============================================================ */
(function () {
  if (window.__mountainAnchorV4) return;
  window.__mountainAnchorV4 = true;

  // gora.png: 1170 x 1730, peak coords: (287,158)
  const IMG_W = 1170, IMG_H = 1730;
  const PEAK_X = 287, PEAK_Y = 158;

  // sled.png: 512 x 512, contact point (полозья): (140,320)
  const SLED_W = 512, SLED_H = 512;
  const CONTACT_FX = 140 / SLED_W; // 0.2734375
  const CONTACT_FY = 320 / SLED_H; // 0.625

  let appliedForThisLift = false;

  function getStableViewportH() {
    // ваш vh-fix: CSS var --vh = 1vh в px (из visualViewport.height)
  ...
  function getStableViewportW() {
    // ширина для backgroundPosition в px
  ...
  function applyMountainToSledContact() {
    ...
    // Контакт: точка полозьев внутри bbox санок (учитывает прозрачные поля png)
    ...
    // Масштаб: от вершины до низа экрана
    let s = (VH - yS) / denom;
    ...
    // Смещение: вершина горы -> в точку контакта полозьев
    let ox = xS - PEAK_X * s;
    let oy = yS - PEAK_Y * s;

    // X-clamp: без пустот по бокам (если шире экрана — кроп)
    if (scaledW >= VW) {
      const minOx = VW - scaledW;
      if (ox < minOx) ox = minOx;
      if (ox > 0) ox = 0;
    } else {
      // если вдруг уже экрана — фиксируем слева
      ox = 0;
    }

    // Дожимаем низ строго к низу экрана (устраняем дрейф дробных пикселей)
    const bottomY = oy + scaledH;
    const drift = VH - bottomY;
    if (Math.abs(drift) > 0.5) oy += drift;

    // Inline стили перебивают contain из CSS
  ...
  function onLiftStart() {
    ...
    // iOS/in-app: высота viewport может стабилизироваться в следующий кадр
  ...
  // Надёжный триггер: факт старта горной сцены (body.scene-fly-up)
  ...
  // Страховка: если во время подъёма меняется viewport (адресная строка/поворот)
...
(function () {
  ...
  if (!window.__finalTriggerStarPatched) {
    ...
    const __goraAudio = new Audio("gora.mp3");
