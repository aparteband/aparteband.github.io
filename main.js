...
/* ============================================================
   UNIVERSAL MOUNTAIN ANCHOR V3 (PC + mobile, robust trigger)
   gora.png peak (287,158) -> sled bottom-center.
   Scale rule: from peak down to screen bottom (crop allowed).
   Trigger: when body gets class "scene-fly-up" (start of mountain scene).
============================================================ */
(function () {
  if (window.__mountainAnchorV3) return;
  window.__mountainAnchorV3 = true;
  ...
  function getStableViewportH() {
    // используем ваш vh-fix (CSS var --vh = 1vh в px)
  ...
  function getStableViewportW() {
    // для ширины достаточно clientWidth (стабильно для background-position px)
  ...
  function applyMountainToSled() {
    ...
    // X-clamp: без пустот по бокам (если гора шире экрана — кроп)
    if (scaledW >= VW) {
      const minOx = VW - scaledW;
      if (ox < minOx) ox = minOx;
      if (ox > 0) ox = 0;
    } else {
      // если гора уже экрана — фиксируем левый край
      ox = 0;
    }

    // Подгоняем низ строго к низу экрана (на случай дробных пикселей/адресной строки)
    const bottomY = oy + scaledH;
    const drift = VH - bottomY;
    if (Math.abs(drift) > 0.5) oy += drift;

    // Применяем (inline перезапишет contain из CSS)
  ...
  function onLiftStart() {
    ...
    // Применяем и делаем 1–2 безопасных повторных прогона:
    // на мобильных высота viewport может стабилизироваться в следующий кадр.
    const ok = applyMountainToSled();
    if (!ok) return;

    appliedForThisLift = true;

    requestAnimationFrame(() => applyMountainToSled());
    setTimeout(() => applyMountainToSled(), 80);
  }

  function onLiftEndOrReset() {
    // Сбросим флаг, чтобы на следующем запуске горы расчёт снова применился
    appliedForThisLift = false;
  }

  // Надёжный триггер: изменение class у <body>
  const mo = new MutationObserver(() => {
    ...
    // рестарт/сброс сцены
    if (!fly || b.classList.contains("scene-reset")) onLiftEndOrReset();
  });
  ...
  // Доп.страховка на iOS: если во время подъёма меняется viewport — пересчитаем (только пока не hero-ready)
