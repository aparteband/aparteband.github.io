...
/* MOBILE SCALE: звёзды и разнесение */
const __IS_MOBILE_UI =
  !!(window.matchMedia && window.matchMedia("(hover: none) and (pointer: coarse)").matches);

/* ВАЖНО: звёзды на мобиле ещё +2× (итого 4× от исходного) */
const __STAR_MOBILE_SCALE = __IS_MOBILE_UI ? 4 : 1;

/* Иконки на мобиле ещё +2× (итого 4× от исходного) */
const __ICON_MOBILE_SCALE = __IS_MOBILE_UI ? 4 : 1;
...
function createStars() {
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    /* комфортное разнесение на мобиле: gap и keepOut масштабируем */
...
function showIconsFan() {
  ...
  /* ВАЖНО: чтобы при увеличении иконок они НЕ слипались —
     сохраняем “шаг” веера как в начале (масштабируем spread) */
  const spread = 360 * __ICON_MOBILE_SCALE;
