...
function createStars() {
  ...
  if (__IS_MOBILE_UI) {
    ...
    if (lay) {
      ...
      window.__iconsNoStarZonePx = noStar; // отладка (не влияет на логику)
...
function __iconsFan_computeMobileLayout() {
  ...
  // базовый размер: W/7.5 (≈ 20% меньше, чем W/6)
  let S = Math.floor(W / 7.5);

  // защитный минимум на очень узких экранах
  if (!isFinite(S) || S < 34) S = 34;

  // считаем gap из точного условия “поля = 0.5S”
  let G = Math.floor((W - (n + 1) * S) / Math.max(1, (n - 1)));

  // gap — единственное, что можно “подкрутить”: держим минимально комфортный
  const MIN_GAP = 14;
  if (!isFinite(G)) G = MIN_GAP;

  if (G < MIN_GAP) {
    // уменьшаем S так, чтобы gap стал >= MIN_GAP при том же правиле полей
    const S2 = Math.floor((W - (n - 1) * MIN_GAP) / (n + 1));
    if (isFinite(S2) && S2 > 0) S = S2;
    G = Math.floor((W - (n + 1) * S) / Math.max(1, (n - 1)));
    if (!isFinite(G) || G < 0) G = 0;
  }

  const step = S + G;

  // центрируем по “середина иконки = центр экрана”
  const baseLeft = (W / 2) - (S / 2);

  // поднимаем выше, чтобы не резалось панелями/поисковой строкой
  // (высота берётся из --vh / visualViewport)
  const bottomPad = Math.max(24, Math.round(S * 1.05));
  let top = Math.round(H - (S + bottomPad));

  // страховка от ухода слишком низко/вверх
  top = Math.max(24, Math.min(top, Math.round(H - S - 10)));

  return { W, H, S, G, step, baseLeft, top };
}
...
function showIconsFan() {
  ...
  ICONS.forEach((src, i) => {
    ...
    // ONLY MOBILE: чтобы bottom из CSS не конфликтовал с top (и не было “обрезаний”/растяжек)
    if (isMobile) {
      icon.style.bottom = "auto";
      if (mobileLay) {
        icon.style.width = mobileLay.size + "px";

        // симметричный шаг => центрирование по крайним соблюдается
        icon.style.setProperty("--tx", (offset * mobileLay.step) + "px");
      } else {
