...
/* ============================================================
   MOBILE MOUNTAIN ANCHOR (phone-only)
   Peak of gora.png (287,158) -> sled bottom-center.
   Scale rule: from peak down to screen bottom (no contain).
============================================================ */
(function () {
  if (window.__mobileMountainAnchorV1) return;
  window.__mobileMountainAnchorV1 = true;

  // gora.png: 1170 x 1730, peak coords from Paint: (287,158)
  const IMG_W = 1170, IMG_H = 1730;
  const PEAK_X = 287, PEAK_Y = 158;

  function isPhoneLike() {
    // “только телефоны”: грубый указатель + небольшой min-габарит
    return (
      window.matchMedia &&
  ...
  function getStableViewportH() {
    // используем тот же источник, что и ваша vh-fix: CSS var --vh (в px за 1vh)
    const vh = parseFloat(
  ...
  function applyMountainToSled() {
    if (!isPhoneLike()) return;

    const mountain = document.getElementById("mountain-scene");
    const sled = document.getElementById("sled");
    if (!mountain || !sled) return;

    const r = sled.getBoundingClientRect();
    if (!isFinite(r.left) || r.width < 1 || r.height < 1) return;

    const VW = document.documentElement.clientWidth || window.innerWidth;
    const VH = getStableViewportH();

    // контакт: нижняя середина санок
    ...
    // Смещения так, чтобы вершина попала в (xS,yS)
    let ox = xS - PEAK_X * s;
    let oy = yS - PEAK_Y * s;

    // X-clamp: без пустот по бокам. Если гора шире экрана — допускаем кроп.
    if (scaledW >= VW) {
      const minOx = VW - scaledW; // самый левый допустимый (чтобы правый край не “убежал”)
      if (ox < minOx) ox = minOx;
      if (ox > 0) ox = 0;
    } else {
      // если вдруг гора уже экрана — фиксируем левый край (никакого “центра”)
      ox = 0;
    }

    // микродрифт по Y (числовая точность): гарантируем ровно низ внизу
    const bottomY = oy + scaledH;
    const drift = VH - bottomY;
    if (Math.abs(drift) > 0.5) oy += drift;

    // Применяем как px-size + px-position (перебивает contain)
  ...
  // 1) Важно: применяем ДО вашего обработчика клика по финальной звезде
  document.addEventListener(
    "click",
    (e) => {
      const t = e.target;
      if (!t || !t.closest) return;
      if (!t.closest("#final-star-trigger")) return;
      applyMountainToSled();
    },
    true // capture
  );

  // 2) Поддерживаем корректность при изменении высоты viewport на iOS (адресная строка/поворот)
  function maybeReapply() {
    if (!isPhoneLike()) return;
    if (
      document.body.classList.contains("scene-fly-up") ||
      document.body.classList.contains("hero-ready") ||
      document.body.classList.contains("mountain-drop")
    ) {
      applyMountainToSled();
    }
  }

  window.addEventListener("orientationchange", () => setTimeout(maybeReapply, 60));
  window.addEventListener("resize", () => requestAnimationFrame(maybeReapply));
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", () => requestAnimationFrame(maybeReapply));
  }
})();
