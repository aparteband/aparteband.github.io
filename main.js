...
(function () {
  ...
  function scheduleReleaseIfStartClick(e) {
    const t = e.target;
    if (!t || !t.closest) return;
    if (!t.closest("#" + START_BTN_ID)) return;
    requestAnimationFrame(() => release());
  }

  document.addEventListener("pointerup", scheduleReleaseIfStartClick, true);
  document.addEventListener("click", scheduleReleaseIfStartClick, true);
...
/* ============================================================
   AUDIO PRIME HUB (для стабильности в мобильных браузерах)
   — НЕ меняет тайминги сцен: только снижает шанс "звук не сыграл"
============================================================ */
(function () {
  if (window.__audioPrimeHubV1) return;
  window.__audioPrimeHubV1 = true;
  ...
  /* ============================================================
     FIX 4.1: прайм в заданном порядке (приоритет)
     — порядок вызовов: клики/первый bell/ser+zam → остальные bell → (прочее при необходимости) → gora
     — open.mp3 исключаем через excludeList
  ============================================================ */
  function primeInOrder(list = [], excludeList = []) {
    const exclude = new Set(excludeList || []);
  ...
  window.__registerPrimeAudio = register;
  window.__primeAllAudio = primeAll;
  window.__primeAudioInOrder = primeInOrder;
})();

const starsWrap = document.getElementById("stars");

const STAR_IMAGES = [1,2,3,4,5,6].map(n => `star${n}.png`);
const STAR_COUNT = 20;
const ICONS = ["vk.png", "ya.png", "ap.png", "sp.png", "zv.png"];

/* ============================================================
   INPUT EVENT (FIX 1.2): единый “тап/клик” без дублей
   — iOS 15+ + in-app: используем pointerup если поддерживается, иначе click
============================================================ */
const __TAP_EVT = (window.PointerEvent ? "pointerup" : "click");

/* ============================================================
   DEVICE CAPS (для мобильных оптимизаций без "самодеятельности")
============================================================ */
const __HAS_FINE_POINTER = !!(window.matchMedia && window.matchMedia("(pointer: fine)").matches);
const __HAS_COARSE_POINTER = !!(window.matchMedia && window.matchMedia("(pointer: coarse)").matches);

/* ============================================================
   CSS PATCHES (гора на мобиле + звезды без сброса анимаций)
   — патчим поверх вашего CSS, не ломая остальное
============================================================ */
(function () {
  if (window.__sceneCssPatchV2) return;
  window.__sceneCssPatchV2 = true;

  const STYLE_ID = "scene-css-patch-v2";
  if (document.getElementById(STYLE_ID)) return;

  const st = document.createElement("style");
  st.id = STYLE_ID;
  st.textContent = `
    /* Stars: базовая стабильность + GPU */
    #stars{
      position: fixed;
      inset: 0;
      overflow: hidden;
      pointer-events: none; /* сами звезды включают pointer-events через inline */
      z-index: 5000;
    }
    img.star{
      position: absolute;
    ...
    /* ВАЖНО: glow/glow-max НЕ должны переписывать animation и сбрасывать падение */
    img.star.glow,
    img.star.glow-max,
    img.star.hover{
      animation-name: starFall !important;
    ...
    img.star.glow{
      filter: brightness(2.2) drop-shadow(0 0 14px rgba(255,255,255,0.65));
    }
    img.star.glow-max{
      filter:
        brightness(2.65)
    ...
    /* На всякий случай: если у вас не было keyframes starFall, добавляем безопасный вариант */
    @keyframes starFall { to { top: var(--final-top); } }

    /* Mountain: фиксируем позицию и убираем "центрирование" на мобильных */
    #mountain-scene{
      position: fixed;
      inset: 0;
    ...
    /* Мобильный режим: НЕ contain, а "обрезаем" — показываем правую часть, масштаб стабильный */
    @media (max-width: 900px), (pointer: coarse) {
      #mountain-scene{
        background-position: right bottom !important;
        background-size: auto calc(120 * var(--vh, 1vh)) !important;
      }
    }
  `;
  document.head.appendChild(st);
...
/* ============================================================
   ASSET PRELOAD (images + audio fetch->blob url)
   — вы просили обязательный прелоад: делаем
============================================================ */
(function () {
  if (window.__assetPreloadV2) return;
  window.__assetPreloadV2 = true;

  const __audioBlobUrl = new Map();         // src -> blobURL
  const __audioWaiters = new Map();         // src -> [resolve]
  const __audioInstances = new Map();       // src -> Set(Audio)
  let __audioStarted = false;

  function __getWaiters(src) {
    let arr = __audioWaiters.get(src);
    if (!arr) {
      arr = [];
      __audioWaiters.set(src, arr);
    }
    return arr;
  }

  function __resolveWaiters(src, url) {
    const arr = __audioWaiters.get(src);
    if (!arr || !arr.length) return;
    __audioWaiters.delete(src);
    for (let i = 0; i < arr.length; i++) {
      try { arr[i](url); } catch (_) {}
    }
  }

  function __applyAudioUrlToInstances(src, url) {
    const set = __audioInstances.get(src);
    if (!set || !set.size) return;
    set.forEach(a => {
      try {
        if (!a) return;
        // если уже стоит blobURL — не трогаем
        if (String(a.src || "").includes("blob:")) return;
        a.src = url;
        try { a.load(); } catch (_) {}
      } catch (_) {}
    });
  ...
  function registerAudioInstance(src, audioEl) {
    if (!src || !audioEl) return;
    let set = __audioInstances.get(src);
    if (!set) {
      set = new Set();
      __audioInstances.set(src, set);
    }
    set.add(audioEl);

    const readyUrl = __audioBlobUrl.get(src);
    if (readyUrl) {
      __applyAudioUrlToInstances(src, readyUrl);
    }
  }

  function getAudioUrl(src) {
    return __audioBlobUrl.get(src) || src;
  }

  function whenAudioReady(src) {
    const readyUrl = __audioBlobUrl.get(src);
    if (readyUrl) return Promise.resolve(readyUrl);
    return new Promise(resolve => {
      __getWaiters(src).push(resolve);
    });
  }
  ...
  async function preloadImages(list) {
    ...
    const tasks = uniq.map(src => new Promise(resolve => {
      ...
      img.src = src;
      // decode() не везде, но если есть — помогает убрать "паузы"
      if (img.decode) {
  ...
  async function fetchToBlobURL(src) {
    if (__audioBlobUrl.has(src)) return __audioBlobUrl.get(src);
    try {
      const r = await fetch(src, { cache: "force-cache" });
      if (!r.ok) throw new Error("HTTP " + r.status);
      const b = await r.blob();
      const url = URL.createObjectURL(b);
      __audioBlobUrl.set(src, url);
      __applyAudioUrlToInstances(src, url);
      __resolveWaiters(src, url);
      return url;
    } catch (_) {
      // Не фейлим приложение — просто оставляем как есть
      __resolveWaiters(src, src);
      return src;
    }
  }

  async function preloadAudioInOrder(list) {
    for (let i = 0; i < list.length; i++) {
      await fetchToBlobURL(list[i]);
    }
  }

  // Картинки — обязательные (ваши файлы в одной папке)
  const IMG_LIST = [
    "hero.png",
    "hero-mini.png",
    "sled.png",
    ...
    "sub.png",
    "gora.png",
    "vk.png","ya.png","ap.png","sp.png","zv.png",
    "star1.png","star2.png","star3.png","star4.png","star5.png","star6.png"
  ];

  // Аудио — приоритет как вы требовали
  const AUDIO_ORDER = [
    "open.mp3",
    "cl.mp3",
    "bell1.mp3",
  ...
  // Стартуем сразу
  window.__assetsImagesPromise = preloadImages(IMG_LIST);

  // Аудио тоже стартуем сразу (не ждем жест пользователя для fetch — это разрешено)
  window.__assetsAudioPromise = (async () => {
    if (__audioStarted) return;
    __audioStarted = true;
    await preloadAudioInOrder(AUDIO_ORDER);
  })();

  window.__registerAudioForPreload = registerAudioInstance;
  window.__getAudioUrl = getAudioUrl;
  window.__whenAudioReady = whenAudioReady;
})();

/* ============================================================
   AUDIO: HEART (ser.mp3)
============================================================ */
const SER_SND = new Audio((window.__getAudioUrl ? window.__getAudioUrl("ser.mp3") : "ser.mp3"));
SER_SND.preload = "auto";
SER_SND.loop = false;
try { SER_SND.load(); } catch (_) {}
if (window.__registerPrimeAudio) window.__registerPrimeAudio(SER_SND);
if (window.__registerAudioForPreload) window.__registerAudioForPreload("ser.mp3", SER_SND);
window.__SFX_SER = SER_SND;

function playSerOnce() {
  try {
    SER_SND.pause();
    SER_SND.currentTime = 0;
    const p = SER_SND.play();
...
/* ---------- SFX: ZAMOK (zam.mp3) ---------- */
const ZAM_SND = new Audio((window.__getAudioUrl ? window.__getAudioUrl("zam.mp3") : "zam.mp3"));
ZAM_SND.preload = "auto";
ZAM_SND.loop = false;
try { ZAM_SND.load(); } catch (_) {}
if (window.__registerPrimeAudio) window.__registerPrimeAudio(ZAM_SND);
if (window.__registerAudioForPreload) window.__registerAudioForPreload("zam.mp3", ZAM_SND);
window.__SFX_ZAM = ZAM_SND;

function playZamSfxOnce() {
  try {
    ZAM_SND.pause();
    ZAM_SND.currentTime = 0;
  } catch (_) {}
  const p = ZAM_SND.play();
  if (p && typeof p.catch === "function") p.catch(() => {});
}
...
const __bellBaseBySrc = new Map();
BELL_SOURCES.forEach(src => {
  const a = new Audio((window.__getAudioUrl ? window.__getAudioUrl(src) : src));
  a.preload = "auto";
  try { a.load(); } catch (_) {}
  a.__ok = true;
  a.addEventListener("error", () => { a.__ok = false; }, { once: true });
  __bellBaseBySrc.set(src, a);
  if (window.__registerPrimeAudio) window.__registerPrimeAudio(a);
  if (window.__registerAudioForPreload) window.__registerAudioForPreload(src, a);
});
window.__BELL_BASES = Array.from(__bellBaseBySrc.values());

function __playBellSrc(src) {
  const base = __bellBaseBySrc.get(src);

  if (base && base.__ok && (base.paused || base.ended)) {
  ...
  if (base && base.__ok) {
    const extra = new Audio((window.__getAudioUrl ? window.__getAudioUrl(src) : src));
    extra.preload = "auto";
    const p = extra.play();
    if (p && typeof p.catch === "function") p.catch(() => {});
  }
...
function createStars() {
  ...
  const isSmallScreen = vmin <= 520;

  // Мобильный UX: минимум 44px + масштаб от экрана
  const minSize = isSmallScreen ? Math.max(44, vmin * 0.085) : 30;
  const maxSize = isSmallScreen ? Math.max(minSize + 12, vmin * 0.16) : 80;
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    ...
    let keepOut = FINAL_STAR_KEEP_OUT_PX;
    ...
    /* FIX 5.1: вместо -120vh используем стабильный --vh */
    ...
    // звезды кликаются обычным click (как вы выбрали)
...
function pulseStars(duration = 6000, onDone) {
  ...
  /* OPT 3.3: используем кеш, если он актуален */
...
function showIconsFan() {
  ...
  ICONS.forEach((src, i) => {
    ...
    icon.style.top = y + "px"; // 2.3 не трогаем
    icon.style.setProperty("--tx", offset * spread / ICONS.length + "px");
...
============================================================ */
(function () {
  if (window.__clSfxV1) return;
  window.__clSfxV1 = true;

  const CL_SRC = "cl.mp3";

  const CL_BASE = new Audio((window.__getAudioUrl ? window.__getAudioUrl(CL_SRC) : CL_SRC));
  CL_BASE.preload = "auto";
  try { CL_BASE.load(); } catch (_) {}
  if (window.__registerPrimeAudio) window.__registerPrimeAudio(CL_BASE);
  if (window.__registerAudioForPreload) window.__registerAudioForPreload(CL_SRC, CL_BASE);
  window.__SFX_CLICK = CL_BASE;

  function playClOnce() {
    try {
      if (CL_BASE.paused || CL_BASE.ended) {
        try { CL_BASE.currentTime = 0; } catch (_) {}
        const p = CL_BASE.play();
        if (p && typeof p.catch === "function") p.catch(() => {});
        return;
      }

      const extra = new Audio((window.__getAudioUrl ? window.__getAudioUrl(CL_SRC) : CL_SRC));
      extra.preload = "auto";
      const p = extra.play();
      if (p && typeof p.catch === "function") p.catch(() => {});
    } catch (_) {}
...
/* ============================================================
   FIX: ИКОНКИ ОТКРЫВАЮТСЯ ТОЛЬКО В НОВОЙ ВКЛАДКЕ
   — и НЕ должны дополнительно открываться в текущей
   — плюс гасим “синтетический click” после pointerup (двойное срабатывание)
============================================================ */
(function () {
  if (window.__iconOpenFixV2) return;
  window.__iconOpenFixV2 = true;

  let __lastNavTs = 0;

  function fallbackBlocked(url) {
    // НЕЛЬЗЯ уходить в текущую вкладку: вместо этого — копирование/подсказка
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(url).then(() => {
          alert("Браузер заблокировал новую вкладку. Ссылка скопирована в буфер обмена.");
        }).catch(() => {
          prompt("Браузер заблокировал новую вкладку. Скопируйте ссылку:", url);
        });
      } else {
        prompt("Браузер заблокировал новую вкладку. Скопируйте ссылку:", url);
      }
  ...
  function openInNewTab(url, e) {
    const now = Date.now();
    if (now - __lastNavTs < 450) {
      if (e) {
    ...
    if (!w) fallbackBlocked(url);
  }

  // Открытие по pointerup
  document.addEventListener(__TAP_EVT, (e) => {
    ...
    for (const key in ICON_LINKS) {
      if (src.includes(key)) {
        openInNewTab(ICON_LINKS[key], e);
        break;
      }
    }
  }, true);

  // Поглощаем click, чтобы не было второго срабатывания после pointerup
...
// На тач-устройствах mousemove и starForces не нужны (и дают дёрганье)
if (__HAS_FINE_POINTER) {
...
function updateStarForces() {
  // На мобильных полностью отключаем этот RAF-цикл
  if (!__HAS_FINE_POINTER) return;
...
/* ---------- СТАРТ (BOOT SCREEN + open.mp3) ---------- */
(function () {
  if (window.__bootScreenV1) return;
  ...
  let __preStartRAF = 0;
  let __clampLastTs = 0; /* FIX 3.1: троттлинг */
  ...
  function preStartClampTick(ts) {
    ...
    /* FIX 3.1: тяжелые сбросы выполняем не каждый кадр */
  ...
  function ensureStyle() {
    ...
    st.textContent = `
      ...
      #${BTN_ID}{
        ...
        animation: bootBtnPulse 2.2s ease-in-out infinite;
      }
      @keyframes bootBtnPulse{
  ...
  function startFirstScene() {
    ...
    /* маркер “приложение стартовало” для BFCache (FIX 1.1) */
  ...
  const audio = new Audio((window.__getAudioUrl ? window.__getAudioUrl("open.mp3") : "open.mp3"));
  audio.preload = "auto";
  audio.loop = false;
  audio.volume = 1;
  try { audio.load(); } catch (_) {}
  if (window.__registerAudioForPreload) window.__registerAudioForPreload("open.mp3", audio);
  ...
  async function onStart(e) {
    ...
    /* ============================================================
       FIX 4.1: приоритетный прайм (open.mp3 исключаем)
       Порядок:
       1) click (cl)
       2) один bell (base[0])
       3) ser + zam
    ...
    try {
      ...
      const bellBases = (window.__BELL_BASES && window.__BELL_BASES.length)
        ? window.__BELL_BASES
        : [];

      const bellFirst = bellBases[0] || null;
      const bellRest = bellBases.slice(1);

      const batch1 = [
        window.__SFX_CLICK || null,
        bellFirst,
        SER_SND,
        ZAM_SND
      ].filter(Boolean);

      const batch2 = bellRest.filter(Boolean);

      const batchLast = [
        window.__SFX_GORA || null
      ].filter(Boolean);

      if (window.__primeAudioInOrder) {
        window.__primeAudioInOrder(batch1, exclude);
        window.__primeAudioInOrder(batch2, exclude);
        window.__primeAudioInOrder(batchLast, exclude);
      } else if (window.__primeAllAudio) {
        window.__primeAllAudio(exclude);
      }
    } catch (_) {}

    // Пытаемся гарантировать "open" без заиканий: если blob уже готов — отлично
    try {
      if (window.__whenAudioReady) {
        await Promise.race([
          window.__whenAudioReady("open.mp3"),
...
(function initMountainScene() {
  ...
  // гарантируем, что картинка горы есть даже если CSS где-то потерялся
...
/* --- дальше хвост: sled fade, icon exit, mountain-drop, restartScene, final star, sub --- */
...
(function () {
  ...
  function restartScene() {
    ...
    /* ============================================================
       ...
       — перед пересозданием звёзд сбрасываем флаг переноса иконок в wrap
    ============================================================ */
    ...
    /* на каждый рестарт снова делаем fan + перенос в wrap (как в первом старте) */
...
(function () {
  ...
  if (!window.__finalTriggerStarPatched) {
    ...
    const __goraAudio = new Audio((window.__getAudioUrl ? window.__getAudioUrl("gora.mp3") : "gora.mp3"));
    __goraAudio.preload = "auto";
    try { __goraAudio.load(); } catch (_) {}
    if (window.__registerPrimeAudio) window.__registerPrimeAudio(__goraAudio);
    if (window.__registerAudioForPreload) window.__registerAudioForPreload("gora.mp3", __goraAudio);
    window.__SFX_GORA = __goraAudio;
    ...
    function ensureFinalStarEl() {
      ...
      el.addEventListener("click", (e) => {
        ...
        try {
          __goraAudio.currentTime = 0;
          const p = __goraAudio.play();
          if (p && typeof p.catch === "function") p.catch(() => {});
        } catch (_) {}
...
(function () {
  ...
  function ensureEl() {
    ...
    /* tap: только новая вкладка; если блок — НЕ уходим в текущую */
    el.addEventListener(__TAP_EVT, (e) => {
      e.preventDefault();
      e.stopPropagation();
      const w = window.open(SUB_LINK, "_blank", "noopener,noreferrer");
      if (!w) fallbackBlocked(SUB_LINK);
    }, { passive: false });

    // гасим "второй click" после pointerup
    el.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
