...
function showIconsFan() {
  ...
  const mobile = !!__IS_MOBILE_UI;
  const layout = mobile ? __iconsFan_computeMobileLayout() : null;
  ...
  ICONS.forEach((src, i) => {
    ...
    if (mobile && layout) {
      // мобайл: точная геометрия под экран + центр по крайним + не режется панелями
      icon.style.left = Math.round(layout.baseLeft) + "px";
      icon.style.top = Math.round(layout.top) + "px";
      icon.style.bottom = "";
      icon.style.width = layout.S + "px";
  ...
  // мобайл: после вставки — ещё раз применяем геометрию (и дальше поддерживаем при resize)
  if (mobile) {
    try { window.__repositionIconsFan(); } catch (_) {}
  }
}
