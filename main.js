/* ============================================================
   ...
   — FIX: после старта сцены замораживаем --vh, чтобы адресная строка
     не дергала transform'ы и не запускала сюжет
============================================================ */
(function () {
  ...
  function freezeIfStarted() {
    // после старта — фиксируем vh и больше не меняем на address-bar / search / keyboard
    if (!window.__appStarted) return false;

    if (!frozenH) {
      // фиксируем на "самую большую видимую высоту", чтобы не ловить дерганья
      frozenH = maxH || readH();
      if (!maxH || frozenH > maxH) maxH = frozenH;
    }
  ...
  function onAnyResizeLike() {
    // если сцена уже запущена — держим vh замороженным
    if (freezeIfStarted()) return;
    updateLive();
  }

  // первичная установка (до старта — live/max, после старта — freeze)
  updateLive();

  window.addEventListener("resize", onAnyResizeLike);
  window.addEventListener("orientationchange", () => {
    // ориентация = реальная смена геометрии => сбрасываем freeze
    frozenH = 0;
    maxH = 0;
    setTimeout(() => {
      updateLive();
  ...
  if (window.visualViewport) {
    window.visualViewport.addEventListener("resize", onAnyResizeLike);
    window.visualViewport.addEventListener("scroll", onAnyResizeLike);
  }

  // возврат из bfcache / in-app прыжки
  window.addEventListener("pageshow", () => {
    // если уже стартовали — просто переапплаим frozen
    if (freezeIfStarted()) return;
    updateLive();
  });
})();
